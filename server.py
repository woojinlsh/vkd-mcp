...
# ==========================================
# 1. MCP 서버 초기화 및 API 설정
# ==========================================
mcp = FastMCP("Verkada_Camera_Alerts")

# 환경변수에서 Verkada API 키 가져오기 (Render의 Environment Variables에 설정된 값)
...
# ==========================================
# 3. 서버 실행 (Render 프록시 환경 완벽 호환)
# ==========================================
if __name__ == "__main__":
    # Render에서 할당한 포트 번호 가져오기 (기본값 8000)
    port = int(os.environ.get("PORT", "8000"))
    
    # FastMCP 내부에 구성된 웹 앱(ASGI) 객체 추출
    app = None
    ...
    if not app:
        print("오류: MCP 서버 앱을 초기화할 수 없습니다.")
        exit(1)

    print(f"Starting MCP Server on 0.0.0.0:{port} using Uvicorn...")
    
    # [핵심] proxy_headers=True 와 forwarded_allow_ips="*" 를 추가하여 Render 보안 정책 통과
    uvicorn.run(
        app, 
        host="0.0.0.0", 
        port=port, 
